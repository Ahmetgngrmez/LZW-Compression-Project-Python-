...
def main():
    ...
    # --- Sıkıştırılmış veriyi binary olarak diske kaydetme ---
    compressed_file_name = f"{base_name}_compressed.bin"
    compressed_file_path = os.path.join(base_path, compressed_file_name)
    
    with open(compressed_file_path, "wb") as f:
        # Kodları 2 byte (16-bit) halinde kaydediyoruz
        for code in compressed_data:
            f.write(code.to_bytes(2, byteorder='big'))
    print(f"Compressed file saved as: {compressed_file_name}")
    ...
    # --- Diske kaydedilen sıkıştırılmış dosyayı okuma ---
    print("Reading compressed data from file...")
    read_compressed_data = []
    with open(compressed_file_path, "rb") as f:
        while True:
            bytes_read = f.read(2) # 2 byte oku
            if not bytes_read:
                break
            read_compressed_data.append(int.from_bytes(bytes_read, byteorder='big'))

    # 4. Decompression Process (Hafızadaki değil, dosyadan okunan data kullanılıyor)
    print("Decompressing...")
