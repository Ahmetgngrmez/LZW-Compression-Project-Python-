...
class MainWindow(QMainWindow):
    ...
    def compress_file(self):
        ...
        if method == "Level 1" and file_ext == ".txt":
            ...
            self.update_stats(ent, avg, orig_s/comp_s if comp_s > 0 else 0, orig_s, comp_s)
            return
        ...
        if file_ext in [".jpg", ".jpeg", ".png", ".bmp"]:
            ...
            if method == "Level 2":
                ...
                r_c = LZW_Level4.lzw_compress(r)
                g_c = LZW_Level4.lzw_compress(g)
                b_c = LZW_Level4.lzw_compress(b)
                
                # Tüm kanalların metriklerini toplayıp ortalamasını alıyoruz
                e_r, a_r, _, s_r = LZW_Level4.calculate_metrics(r, r_c)
                e_g, a_g, _, s_g = LZW_Level4.calculate_metrics(g, g_c)
                e_b, a_b, _, s_b = LZW_Level4.calculate_metrics(b, b_c)
                
                ent = (e_r + e_g + e_b) / 3
                avg = (a_r + a_g + a_b) / 3
                t_size = s_r + s_g + s_b
                rat = (len(r) * 8 * 3) / t_size if t_size > 0 else 0
                ...
                res_comp = []; t_ent = 0; t_avg = 0; t_size = 0
                for ch in [r, g, b]:
            ...
            # Hata olmaması için if bloğu
            if not final_data_to_save: return
    ...
    def decompress_file(self):
        ...
        if method == "Level 2":
            ...
            # self.diff_shape yerine doğrudan height, width kullanımı
            final = LZW_Level3.restore_from_difference(np.array(pix).reshape((self.height, self.width)))
            restored_img = Image.new("L", (self.width, self.height))
